...
run_setup() {
    ...
    read -rp "  Webhook URL [${existing_webhook:-none}]: " input
    ...
    read -rp "  Disk device [${existing_disk_device}, detected: ${detected_device}]: " input
    DISK_DEVICE="${input:-$existing_disk_device}"

    # --- CPU/RAM alerts ---
    echo ""
    info "CPU/RAM alert thresholds (%)."
    read -rp "  CPU alert threshold  [${existing_cpu_thresh}]: " input
    CPU_ALERT_THRESHOLD="${input:-$existing_cpu_thresh}"
    read -rp "  RAM alert threshold  [${existing_ram_thresh}]: " input
    RAM_ALERT_THRESHOLD="${input:-$existing_ram_thresh}"
    read -rp "  Sustained minutes before alert [${existing_sustain}]: " input
    CPU_SUSTAIN_MINUTES="${input:-$existing_sustain}"

    # --- Log vacuum ---
    echo ""
    info "Log vacuum settings."
    read -rp "  Directories to vacuum (space-separated) [${existing_vac_dirs}]: " input
    LOG_VACUUM_DIRS="${input:-$existing_vac_dirs}"
    read -rp "  Truncate logs older than N days [${existing_vac_days}]: " input
    ...
    read -rp "  Services [${watchdog_default:-none}]: " input
    WATCHDOG_SERVICES="${input:-$watchdog_default}"
    read -rp "  Auto-restart failed services? [${existing_watchdog_restart}]: " input
    WATCHDOG_AUTO_RESTART="${input:-$existing_watchdog_restart}"

    # --- Swap alert ---
    echo ""
    info "Swap alert threshold (%) and sustained minutes."
    read -rp "  Swap alert threshold [${existing_swap_thresh}]: " input
    SWAP_ALERT_THRESHOLD="${input:-$existing_swap_thresh}"
    read -rp "  Sustained minutes before alert [${existing_swap_sustain}]: " input
    SWAP_SUSTAIN_MINUTES="${input:-$existing_swap_sustain}"

    # --- Tmp cleanup ---
    echo ""
    info "Tmp cleanup: directories and age threshold."
    read -rp "  Directories (space-separated) [${existing_tmp_dirs}]: " input
    TMP_CLEANUP_DIRS="${input:-$existing_tmp_dirs}"
    read -rp "  Delete files not accessed in N days [${existing_tmp_days}]: " input
...
EOF
    ...
    echo ""
    read -rp "Set up cron jobs now? [Y/n]: " yn
    if [[ "${yn,,}" != "n" ]]; then
